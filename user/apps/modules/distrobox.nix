...
in {
  ...
  config = let
    mkBoxes = let
      mkBox = name:
        { img, home ? ".local/share/distrobox/${name}"
        , packages ? [ "git" "neovim" ], init ? "true", flags ? "", path ? [ ]
        , nixPackages ? [ ], symlinks ? [ ], alias ? lib.strings.toLower name
        , exec ? "bash", }: {
    ...
    in attrs: builtins.attrValues (builtins.mapAttrs mkBox attrs);

    mkBoxLinks = box:
      let ln = config.lib.file.mkOutOfStoreSymlink;
      in builtins.listToAttrs (map (link: {
        name = "${box.home}/${link}";
    ...
    mkBoxAlias = box:
      let
        ...
        exec = pkgs.writeShellScript "db-exec" ''
          ...
          ${box.exec}
        '';
      in pkgs.writeShellScriptBin box.alias ''
