...
, aprutil, makeDesktopItem, copyDesktopItems, jq
...
let
  davinci = (stdenv.mkDerivation rec {
    ...
    src = runCommandLocal "${pname}-src.zip" rec {
      ...
      outputHash = if studioVariant then
        "sha256-q11stWFWRDUebAUzGH23R3Spd3EdDG85+6yB/srYCJY="
      else
        "sha256-dYTrO0wpIN68WhBovmYLK5uWOQ1nubpSyKqPCDMPMiM=";
...
in buildFHSEnv {
  ...
  extraInstallCommands = ''
    ...
    ln -s ${davinci}/graphics/DV_Resolve.png $out/share/icons/hicolor/128x128/apps/davinci-resolve${
      lib.optionalString studioVariant "-studio"
    }.png
  '';
