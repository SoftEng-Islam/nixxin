{ settings, pkgs, ... }: {
  ...
  environment.systemPackages = with pkgs; [
    ...
    (if settings.system.features.mediaPlayers.mpv then
      mpv.override { scripts = [ mpvScripts.mpris ]; }
    else
      "")

    # Simple GTK frontend for the mpv video player
    (if settings.system.features.mediaPlayers.celluloid then celluloid else "")

    # Cross-platform media player and streaming server
    (if settings.system.features.mediaPlayers.vlc then vlc else "")

    # A GNOME media player built using GTK4 toolkit and powered by GStreamer with OpenGL rendering
    (if settings.system.features.mediaPlayers.clapper then clapper else "")

    # Linux/macOS media player based on GStreamer and GTK
    (if settings.system.features.mediaPlayers.glide then
      glide-media-player
    else
      "")
