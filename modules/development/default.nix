...
let
  ...
  _pkgs = with pkgs; [
    ...
    (optional settings.modules.development.apps.beekeeper beekeeper-studio)
    # Universal SQL Client for developers, DBA and analysts. Supports MySQL, PostgreSQL, MariaDB, SQLite, and more
    (optional settings.modules.development.apps.dbeaver dbeaver-bin)
    # DB Browser for SQLite
    (optional settings.modules.development.apps.sqlitebrowser sqlitebrowser)
    # Open-source IDE For exploring and testing APIs.
    (optional settings.modules.development.apps.bruno bruno)
    # cross-platform API client for GraphQL, REST, WebSockets, SSE and gRPC. With Cloud, Local and Git storage.
    (optional settings.modules.development.apps.insomnia insomnia)
  ];
in mkIf (settings.modules.development.enable or false) {
  imports = [ ./android ./databases ./languages ./tools ./web ];
