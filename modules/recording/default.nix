...
let
  inherit (lib) mkIf;
  _imports = [
    (lib.optional settings.modules.recording.screen.enable ./screen)
    (lib.optional settings.modules.recording.sound.enable ./sound)
  ];
in mkIf (settings.modules.recording.enable) {
