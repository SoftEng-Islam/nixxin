{ lib, stdenv, callPackage, vscode-generic, fetchurl, buildFHSEnv
, writeShellScript, coreutils, commandLineArgs ? ""
, useVSCodeRipgrep ? stdenv.hostPlatform.isDarwin, }:

let
  inherit (stdenv) hostPlatform;

  sources = (lib.importJSON ./sources.json)."${hostPlatform.system}" or (throw
    "antigravity: unsupported system ${hostPlatform.system}");

  version = "1.11.3";
  vscodeVersion = "1.104.0";
in callPackage vscode-generic {
  inherit commandLineArgs useVSCodeRipgrep version vscodeVersion;
